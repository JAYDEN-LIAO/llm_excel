...
class Executor:
    ...
    def _execute_operation(self, op: Operation) -> OperationResult:
        ...
        if isinstance(op, AggregateOperation):
            ...
            return self._execute_merge(op)
        elif isinstance(op, CrossFileReferenceOperation):
            return self._execute_cross_file_reference(op)
        else:
    ...
    def _execute_join(self, op: JoinOperation) -> OperationResult:
        """执行表连接操作"""
        try:
            import pandas as pd
            
            # 获取左右表数据
            left_table = self.tables.get_table(op.left_file_id, op.left_table)
            right_table = self.tables.get_table(op.right_file_id, op.right_table)
            
            left_df = left_table.get_data()
            right_df = right_table.get_data()
            ...
            for condition in op.conditions:
                ...
                if left_col not in left_df.columns:
                    return OperationResult(
                        operation=op,
                        error=f"左表不存在列: {left_col}"
                    )
                if right_col not in right_df.columns:
                    return OperationResult(
                        operation=op,
                        error=f"右表不存在列: {right_col}"
                    )
                
                merge_on.append((left_col, right_col))
            
            # 执行连接
            if len(merge_on) == 1:
                # 单列连接
                left_on, right_on = merge_on[0]
                if op.join_type == "inner":
                    result_df = pd.merge(left_df, right_df, left_on=left_on, right_on=right_on, how="inner")
                elif op.join_type == "left":
                    result_df = pd.merge(left_df, right_df, left_on=left_on, right_on=right_on, how="left")
                elif op.join_type == "right":
                    result_df = pd.merge(left_df, right_df, left_on=left_on, right_on=right_on, how="right")
                elif op.join_type == "full":
                    result_df = pd.merge(left_df, right_df, left_on=left_on, right_on=right_on, how="outer")
                else:
                    return OperationResult(
                        operation=op,
                        error=f"不支持的连接类型: {op.join_type}"
                    )
            else:
                # 多列连接（简化处理）
                left_on = [col[0] for col in merge_on]
                right_on = [col[1] for col in merge_on]
                result_df = pd.merge(left_df, right_df, left_on=left_on, right_on=right_on, how=op.join_type)
            
            # 处理输出
            output_type = op.output.get("type", "new_sheet") if op.output else "new_sheet"
            if output_type == "new_sheet":
                output_name = op.output["name"]
                output_file_id = op.left_file_id  # 默认输出到左文件
            else:
                output_name = op.left_table
                output_file_id = op.left_file_id
    ...
    def _execute_merge(self, op: MergeOperation) -> OperationResult:
        """执行数据合并操作"""
        try:
            import pandas as pd
            ...
            # 获取所有要合并的表
            tables = []
            for file_id, table_name in zip(op.file_ids, op.tables):
                table = self.tables.get_table(file_id, table_name)
                tables.append(table.get_data())
            
            if op.merge_type == "vertical":
                # 垂直合并（追加行）
                # 检查列是否一致
                ...
                for i, df in enumerate(tables[1:], 1):
                    if set(df.columns) != columns_set:
                        return OperationResult(
                            operation=op,
                            error=f"表 {i} 的列结构与第一表不一致，无法垂直合并"
                        )
                
                result_df = pd.concat(tables, ignore_index=True)
                
            elif op.merge_type == "horizontal":
                # 水平合并（追加列）
                # 检查行数是否一致
                ...
                if len(set(row_counts)) > 1:
                    return OperationResult(
                        operation=op,
                        error=f"表行数不一致，无法水平合并: {row_counts}"
                    )
                ...
                for i, df in enumerate(tables):
                    ...
                    renamed_tables.append(df)
                
                result_df = pd.concat(renamed_tables, axis=1)
                
            else:
                return OperationResult(
                    operation=op,
                    error=f"不支持的合并类型: {op.merge_type}"
                )
            ...
            if output_type == "new_sheet":
                output_name = op.output["name"]
                output_file_id = op.file_ids[0]  # 默认输出到第一个文件
            else:
                output_name = op.tables[0]
                output_file_id = op.file_ids[0]
    ...
    def _execute_cross_file_reference(self, op: CrossFileReferenceOperation) -> OperationResult:
        """执行跨文件引用操作"""
        try:
            # 获取源表数据
            source_table = self.tables.get_table(op.source_file_id, op.source_table)
            source_df = source_table.get_data()
            
            # 验证源列是否存在
            if op.source_column not in source_df.columns:
                return OperationResult(
                    operation=op,
                    error=f"源表不存在列: {op.source_column}"
                )
            
            # 获取目标表数据
            target_table = self.tables.get_table(op.target_file_id, op.target_table)
            target_df = target_table.get_data()
            
            # 获取源列数据
            source_data = source_df[op.source_column].tolist()
            
            if op.reference_type == "copy":
                # 复制数据
                if len(source_data) != len(target_df):
                    return OperationResult(
                        operation=op,
                        error=f"数据长度不匹配: 源表 {len(source_data)} 行, 目标表 {len(target_df)} 行"
                    )
                
                # 添加或更新列
                target_df[op.target_column] = source_data
                
                # 生成公式（简单引用）
                formula = f"={op.source_file_id}.{op.source_table}.{op.source_column}"
                
            elif op.reference_type == "formula":
                # 生成公式引用
                # 这里简化处理，实际Excel公式会更复杂
                formula = f"=INDEX('{op.source_file_id}'!{op.source_column}, ROW())"
                
                # 对于公式类型，我们只记录公式，不实际复制数据
                # 在实际Excel中，公式会在打开时计算
                target_df[op.target_column] = [formula] * len(target_df)
                
            else:
                return OperationResult(
                    operation=op,
                    error=f"不支持的引用类型: {op.reference_type}"
                )
            ...
            return OperationResult(
                ...
                value={
                    ...
                    "formula": formula if op.reference_type == "formula" else None
                }
            )
