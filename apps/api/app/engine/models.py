...
class JoinOperation:
    """
    表连接操作（模拟 Excel 的 XLOOKUP/VLOOKUP 跨文件功能）
    ...
    output: Dict[str, Any]  # {"type": "new_sheet", "name": "连接结果"}
    description: Optional[str] = None
    
    def __post_init__(self):
        if self.join_type not in {"inner", "left", "right", "full"}:
            raise ValueError(f"join_type 必须是 'inner', 'left', 'right' 或 'full'")
        for condition in self.conditions:
            if condition.operator not in {"=", ">", "<", ">=", "<=", "<>"}:
                raise ValueError(f"operator 必须是 '=', '>', '<', '>=', '<=', '<>'")


@dataclass
class MergeOperation:
    """
    数据合并操作（模拟 Excel 的 VSTACK/HSTACK 跨文件功能）
    
    支持垂直合并（追加行）和水平合并（追加列）
    """
    file_ids: List[str]  # 要合并的文件列表
    tables: List[str]  # 对应的表名列表
    merge_type: str  # "vertical"（垂直合并）或 "horizontal"（水平合并）
    output: Dict[str, Any]  # {"type": "new_sheet", "name": "合并结果"}
    description: Optional[str] = None
    
    def __post_init__(self):
        if self.merge_type not in {"vertical", "horizontal"}:
            raise ValueError(f"merge_type 必须是 'vertical' 或 'horizontal'")
        if len(self.file_ids) != len(self.tables):
            raise ValueError(f"file_ids 和 tables 长度必须相同")


@dataclass
class CrossFileReferenceOperation:
    """
    跨文件引用操作（模拟 Excel 的外部引用）
    ...
    target_file_id: str
    target_table: str
    target_column: str  # 新列名或现有列名
    reference_type: str = "copy"  # "copy"（复制数据）或 "formula"（生成公式引用）
    output: Optional[Dict[str, Any]] = None  # 默认 in_place
    description: Optional[str] = None
    
    def __post_init__(self):
        if self.reference_type not in {"copy", "formula"}:
            raise ValueError(f"reference_type 必须是 'copy' 或 'formula'")
        if self.output is None:
            self.output = {"type": "in_place"}
